    ...
    <section>
      ...
      <section>
        <div class="container my-5">
          <header class="mb-4">
              <?php
                $nameType = DB::table('subcategory')
                            ->join('categoryproduct', 'subcategory.CategoryId', '=', 'categoryproduct.Id')
                            ->where('subcategory.Id', $subtypes[0]->Id)
                            ->where('categoryproduct.Id', $type)
                            ->select( 'categoryproduct.Name as Name')
                            ->first();  
                
                $nameSubType = DB::table('subcategory')
                            ->where('subcategory.Id', Route::current()->parameter('subtype'))
                            ->select( 'subcategory.Name as SubName')
                            ->first();
              ?>
            <h5 class="text-primary">
              {{ $nameType->Name }} > {{ $nameSubType->SubName }}
            </h5>
            <h3>Products</h3>
          </header>
    ...
    <script>
      ...
      links.forEach((link) => {
        link.addEventListener("click", () => {
          const linkId = link.dataset.link; // Get the link ID from the "data-link" attribute
          localStorage.setItem("activeLink", linkId); // Set the active link ID in local storage
        });
      });

      // Check local storage for the active link ID and highlight the corresponding link
      const activeLinkId = localStorage.getItem("activeLink");
      if (activeLinkId) {
        const activeLink = sidebar.querySelector(`[data-link="${activeLinkId}"]`);
        activeLink.style.backgroundColor = "#ccc";
